...
if test -f "$TFLITE_FILE"; then
    ...
    curl -o ${TFLITE_FILE} https://tfhub.dev/google/lite-model/yamnet/classification/tflite/1?lite-format=tflite
...
if test -f "$TFLITE_FILE"; then
    ...
    curl -o ${TFLITE_FILE} https://storage.googleapis.com/download.tensorflow.org/models/tflite/speech_commands.tflite
